...
规划器会优先在同一区域内连续活动。例如上午游览城北相邻景点，下午再向城中移动，而不是在城北、城南之间反复往返。对于郊区、山岳等交通成本较高的景点，会尽量单独成组，并把返程时间纳入最后一天检查。
...
> 📌 图片用于展示 TravelPilot 能处理的城市文化、山岳及跨区域旅行场景，不代表固定路线。Agent 会根据用户日期、出发地、同行人和实时信息重新规划。
...
- 💰 预算分类及合计一致性检查；
- 🔄 跨模型、跨 Agent 交接结构检查；
- 📦 源码、安装目录和发布 ZIP 的版本一致性检查；
- 🧪 Gold 与 held-out 评测集及 GitHub Actions 自动测试。
